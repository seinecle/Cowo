...
// TODO: if it's rotted and not used anywhere, can we just get rid of it all?
...
public class AmbiguityClasses {
  ...
  protected int getClass(String word, Dictionary dict, int veryCommonWordThresh, TTags ttags) {
    ...
    if (dict.sum(word) > veryCommonWordThresh) {
      a = new AmbiguityClass(word, true, dict, ttags);
    } else {
      a = new AmbiguityClass(word, dict, ttags);
    }
    return add(a);
  }
